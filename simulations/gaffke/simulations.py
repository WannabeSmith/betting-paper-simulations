...
figures_location = os.path.relpath("./simulations/gaffke/figures/")
...
cs_list = [
    ...
    ConfseqToPlot(
        lambda x: gaffke_ci_seq(
            x, times=times, alpha=alpha, n_uprime=1000, parallel=True
        ),
        "Gaffke",
        "tab:red",
        "-.",
...
dgp_list = [
    ...
    # DataGeneratingProcess(
    #     data_generator_fn=lambda: np.random.permutation(np.append(np.repeat(0.2, int(N/2)), np.repeat(0.51, N-int(N/2)))),
